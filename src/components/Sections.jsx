...
export function EchoesSection({ feature }) {
	return (
		<section className="echoes-section section-shell" id="echoes" aria-labelledby="echoes-title">
			...
			<a className="echoes-feature" href={feature.primaryHref} target="_blank" rel="noreferrer" aria-label={feature.links[0]?.label ?? feature.title} data-reveal>
				<MediaFrame image={feature.primaryImage} accent="#e82bb7" fit="cover" />
				<span className="image-light-sweep" data-scroll-light aria-hidden="true" />
...
export function StudioSection({ copy, feature }) {
	...
	return (
		<section className="studio-section section-shell" id="studio" aria-labelledby="studio-title">
			<header className="studio-masthead">
				<div className="studio-masthead__identity" data-reveal>
					<div className="studio-masthead__logo">{logoSource ? <img src={logoSource} alt={feature.logo.alt} /> : <span>FI</span>}</div>
					<div>
						<p>{copy.eyebrow}</p>
				...
				<div className="studio-masthead__statement" data-reveal>
					<span aria-hidden="true" style={{ rotate: "90deg", translate: "50% 1600%" }} />
			...
			<article className="studio-project" data-reveal>
				<div className="studio-project__art">
					<img src={imageSource} alt={feature.primaryImage.alt} loading="lazy" decoding="async" />
					<span className="image-light-sweep" data-scroll-light aria-hidden="true" />
...
export function ExperienceSection({ copy, items }) {
	return (
		<section className="experience-section section-shell" id="experience" aria-labelledby="experience-title">
			<SectionHeading eyebrow={copy.eyebrow} title={copy.title} id="experience-title" />
...
export function AboutSection({ copy, education, webTools, gameTools }) {
	...
	return (
		<section className="about-section section-shell" id="about" aria-labelledby="about-title">
			<SectionHeading eyebrow={copy.eyebrow} title={copy.title} id="about-title" />

			<div className="about-grid">
				<div className="tool-panel" data-reveal>
					<div className="tool-tabs" role="tablist" aria-label="Tool category">
						<span className={`tool-tabs__indicator ${activeTools !== "game" ? "is-game" : ""}`} aria-hidden="true" />
						<button type="button" role="tab" aria-selected={activeTools === "game"} className={activeTools === "game" ? "is-active" : ""} onClick={() => setActiveTools("game")}>
							{copy.gameLabel}
						</button>
						<button type="button" role="tab" aria-selected={activeTools === "web"} className={activeTools === "web" ? "is-active" : ""} onClick={() => setActiveTools("web")}>
							{copy.webLabel}
						</button>
					</div>
		...
		<section className="contact-section" id="contact" aria-labelledby="contact-title">
			...
			<div className="contact-section__inner section-shell">
				...
				<div className="contact-links" data-reveal>
					...
					<a href="https://github.com/StarPlatinumSan" target="_blank" rel="noreferrer">
						<span>{copy.github}</span>
						<small>@StarPlatinumSan</small>
						<ArrowUpRight />
					</a>
					<a href="https://www.linkedin.com/in/andrei-bituleanu-65b1832aa/" target="_blank" rel="noreferrer">
